...
DERVAL(2) = ((x(4)^2)*x(3)*(N*R+R*J)-x(3)*(N*D+R*H)+(x(3)^2)*(M_a*e_ga*sin(60*t)*60-g*A)...
 *P-(x(3)^2)*x(1)*C*P+x(1)*(R*G-C*N)+P*R*(x(4)^2)*(x(3)^3)+(M_a*e_ga*sin(60*t)*60-g*A)*N-R*...
    (l_pgb*M_a*e_ga*sin(60*t)*3600-l_pgb*M_a*g))*(A*N+A*P*(x(3)^2)-R*F)^(-1);

DERVAL(3) = x(4);

DERVAL(4) = ((x(4)^2)*x(3)*(A*J+F*R)+x(3)*(A*H-D*F)+x(1)*(A*G-C*F)+(M_a*e_ga*sin(60*t)*60-g*A)*F-A*...
